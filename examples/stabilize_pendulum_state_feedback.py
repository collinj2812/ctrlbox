...
from control.state_feedback import StateFeedbackController
...
angle_set = 2 * np.pi
controller = StateFeedbackController(A=system.jac_x, B=system.jac_u, eigvals=np.array([-1e-14, -1e-10, -1e-6, -1e-9]), setpoint=np.array([1, 0, 2 * np.pi, 0]))

print(f'Eigenvalues of A_BK: {np.linalg.eigvals(system.jac_x-system.jac_u@controller.K)}')
...
ani = animate_pendulum(t[::skip], x[::skip], system.default_params()['l'], u[::skip], './animations/pendulum_state_feedback.gif')
